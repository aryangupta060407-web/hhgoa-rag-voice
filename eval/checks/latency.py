...
def run(results: list[ExampleResult]) -> dict:
    ...
    embed_ms = [r.embed_ms_en for r in usable]
    search_ms = [r.search_ms_en for r in usable]
    if usable and usable[0].example.target_language != "en":
        embed_ms += [r.embed_ms_target for r in usable]
        search_ms += [r.search_ms_target for r in usable]
