...
export default function Home() {
  ...
  return <div className="min-h-screen noise-grid">
    ...
    <main className="mx-auto max-w-7xl px-5 py-7 lg:px-8 lg:py-10">
      <section className="mb-8 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="mb-2 font-mono text-xs uppercase tracking-[0.2em] text-primary">HH Goa 2026 · Task 02</p><h1 className="max-w-3xl text-3xl font-extrabold leading-tight tracking-tight sm:text-4xl">Grounded answers from voice.<br /><span className="text-primary">No generation. No guesswork.</span></h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">A deterministic, evidence-first retrieval pipeline with offline multi-strategy chunking, hybrid in-memory retrieval, extractive answer selection, and visible stage-level latency.</p></div><Button onClick={() => benchmark.mutate()} disabled={busy} variant="outline" className="h-10 border-primary/30 bg-primary/5 px-4 text-xs text-primary hover:bg-primary/15 hover:text-primary"><Activity size={15} />{benchmark.isPending ? "Running benchmark…" : "Run 24-query cold benchmark"}</Button></section>
