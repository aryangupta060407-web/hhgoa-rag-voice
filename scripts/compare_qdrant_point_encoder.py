...
def main():
    ...
    encoded = np.array(list(TextEmbedding(model_name=args.model).embed([f"{args.prefix}{text}"]))[0], dtype=np.float32)
    cosine = float(np.dot(stored, encoded) / (np.linalg.norm(stored) * np.linalg.norm(encoded)))
    print(json.dumps({"pointId": args.point_id, "model": args.model, "prefix": args.prefix, "dimension": len(stored), "cosine": round(cosine, 6), "text": text[:240]}, ensure_ascii=False, indent=2))
